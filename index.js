  ...
  table(table, options, callback) {
    return new Promise((resolve, reject) => {
      try {
        ...
        let safelyMarginBottom =
          this.page.margins.top / 2 + (options.marginBottom ?? 0);
        ...
        table.datas.forEach((row, i) => {
          ...
          if (
            ...
            rowBottomY + safelyMarginBottom + rowHeight >= maxY &&
            !lockAddPage
          )
            onFirePageAdded(); // this.emitter.emit('addPage'); //this.addPage();
        ...
        table.rows.forEach((row, i) => {
          ...
          // else this.emitter.emit('addPage'); //this.addPage();
          const tempY =
            this.y + safelyMarginBottom + rowHeight + options.marginBottom;
          if (options.useSafelyMarginBottom && tempY >= maxY && !lockAddPage)
            onFirePageAdded(); // this.emitter.emit('addPage'); //this.addPage();
          ...
          row.forEach((cell, index) => {
            ...
            if (typeof table.headers[index] === 'object') {
              ...
              table.headers[index].fontName &&
                (fontName = table.headers[index].fontName);
            }
